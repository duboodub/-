# -*- coding: utf-8 -*-
"""
Created on Fri Oct  8 23:38:22 2021

    ivp Euler
    scipy.optimize.root (multidimensional)
...
def f_miu_p_1(p,  K1=0.01, K2=1.5, p0=0.1, inverse=False):
...
def solve_miu_ODE(miu0, miu1, x_list, iniside=[0,1]):
    if isinstance(iniside[0],(int)) and isinstance(iniside[1],(int)) and (iniside[1] - iniside[0])==1:
        miu_list = np.zeros(len(x_list))
        miu_list[iniside[0]]= miu0
        miu_list[iniside[1]]= miu1
        ...
        raise ValueError('you did not suitably define a initial side.')
...
def funsv(p, x_i_list,):
    l = len(x_i_list)
    h =  int(l/2)
    iniside = [h-1, h]
        
    miu_list = solve_miu_ODE(f_miu_p_1(p[0]),f_miu_p_1(p[1]), x_i_list, iniside=iniside)
    p_0 = f_miu_p_1(miu_list[0], inverse=True)
    p_1 = f_miu_p_1(miu_list[-1], inverse=True)
    
    return [p_0-x_i_list[0], p_1 - x_i_list[-1]]
...
if __name__ == '__main__':
    ...
    zr = int(len(x_i_list)/2)
    # sol = root_scalar(funs,args=(x_i_list), bracket=[0.3, 0.5], xtol=1e-14,)
    # p1 = sol.root
    # miu_list = solve_miu_ODE(f_miu_p_1(0.4),f_miu_p_1(p1), x_i_list, iniside='right')
    # p_list = f_miu_p_1(miu_list, inverse=True)
    
    solv = root(funsv, x0=[0.3,0.1],args=(x_i_list),tol=1e-26,)
    p = solv.x
    miu_list = solve_miu_ODE(f_miu_p_1(p[0]),f_miu_p_1(p[1]), x_i_list, iniside=[ int(len(x_i_list)/2-1), int(len(x_i_list)/2) ])
    p_list = f_miu_p_1(miu_list, inverse=True)
    
    pr_list = p_list[zr-7:zr+7]
